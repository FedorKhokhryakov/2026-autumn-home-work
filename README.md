...
#### 15–16 баллов
Основное задание, аутентификация и persistent Dao выполнены полностью и корректно. Все шесть эндпоинтов HTTP API работают с правильными кодами и заголовками, редирект отдаётся как 301 с Location, невалидный ID и ссылка возвращают 422, несуществующий ID — 404. Наследник AbstractHttpServiceFactory лежит в пакете со своим именем, имеет публичный конструктор без параметров, запускает HttpServer из JDK и зарегистрирован в UrlShortenerServiceFactoryArgumentsProvider.factories. Сервис делегирует хранение в собственную реализацию Dao. Basic-auth разбирается по спеке и возвращает 401 с WWW-Authenticate при неверных кредах, пользователи хранятся в отдельном Dao<String>, POST /internal/users перезаписывает пароль существующего пользователя, все эндпоинты кроме GET /v0/status и POST /internal/users закрыты аутентификацией. Persistent Dao сохраняет данные на диск и переживает рестарт.
#### 13–14 баллов
Все компоненты выполнены верно, но допущены 1–2 мелких недочёта: в одном эндпоинте пропущен Content-Type, DELETE возвращает не 202, в редиректе отсутствует заголовок Location, POST /internal/users при существующем пользователе не перезаписывает пароль, либо persistent Dao сохраняет данные не атомарно.
#### 11–12 баллов
Одна из трёх частей выполнена с заметным недостатком. Например, аутентификация не отдаёт WWW-Authenticate или закрывает не все эндпоинты; persistent Dao сохраняет данные, но не переживает рестарт; либо отсутствует один из шести эндпоинтов основного задания.
#### 9–10 баллов
Основное задание выполнено полностью (все эндпоинты, валидация, наследник AbstractHttpServiceFactory, делегирование в Dao), а аутентификация и persistent Dao отсутствуют. Либо основное задание + частичная аутентификация с ошибками, но без persistent Dao.
#### 7–8 баллов
Основное задание выполнено с 1–2 заметными недостатками: отсутствует один эндпоинт, нет валидации и возврата 422, сервис не делегирует хранение в Dao, либо наследник AbstractHttpServiceFactory не зарегистрирован в провайдере.
#### 5–6 баллов
Реализована только часть основного задания: часть эндпоинтов есть, но наследник AbstractHttpServiceFactory не создан или не запускает HttpServer из JDK, либо UrlShortenerService не работает с Dao, а хранит данные прямо в себе. Persistent Dao и аутентификация отсутствуют.
#### 3–4 балла
Реализован только один компонент на базовом уровне: например, только Dao или только скелет сервиса с 1–2 эндпоинтами. HTTP-протокол не соблюдён, остальное отсутствует.
#### 0–2 балла
Работа не сдана, скопирована, или архитектура фундаментально неверна: нет Dao, данные захардкожены, HttpServer не запускается, структура ответа не соблюдена.
